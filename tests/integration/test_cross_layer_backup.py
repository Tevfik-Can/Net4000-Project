...
This test runs the complete cross-layer monitoring stack:
1. HTTP server with application metrics
2. eBPF TCP monitoring
3. HTTP client generating load
4. Cross-layer correlation

Based on the original simple_test.py but with improved structure.
"""
import time
import threading
import json
...
def run_test(num_requests=10, output_dir='./results/cross_layer'):
    ...
    print(f"  python src/correlation/correlator.py \\")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cross-layer observability integration test"
    )
    parser.add_argument(
        '--requests',
        type=int,
